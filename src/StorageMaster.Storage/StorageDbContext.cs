...
public sealed class StorageDbContext : IAsyncDisposable
{
    ...
    private async Task MigrateAsync(SqliteConnection conn, CancellationToken ct)
    {
        ...
        if (current < DatabaseSchema.CurrentVersion)
        {
            ...
            if (current < 3)
                await ApplyMigrationAsync(conn, DatabaseSchema.V3Statements, 3, ct);

            if (current < 4)
                await ApplyMigrationAsync(conn, DatabaseSchema.V4Statements, 4, ct);

            if (current < 5)
                await ApplyMigrationAsync(conn, DatabaseSchema.V5Statements, 5, ct);

            if (current < 6)
                await ApplyMigrationAsync(conn, DatabaseSchema.V6Statements, 6, ct);

            if (current < 7)
                await ApplyMigrationAsync(conn, DatabaseSchema.V7Statements, 7, ct);
        }
    }
